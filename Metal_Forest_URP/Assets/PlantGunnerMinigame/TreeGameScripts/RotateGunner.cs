...
public class RotateGunner : MonoBehaviour
{
    ...
    //[SerializeField, Range(0, 180)] private float maxRotation;
    ...
    void Update()
    {
        ...
        //transform.forward = originPoint.transform.forward;
    ...
    private void FixedUpdate()
    {
        ...
        if (-(inputManager.GetRotatary3) < currentInput)
        {
            neutralizeDirection = false;
            direction += Time.fixedDeltaTime * rotatationSmoothness;
            currentInput = (-(inputManager.GetRotatary3));
            clock++;
            currentDirection = direction;
            lerpRatio = 0;
        }
        else if (-(inputManager.GetRotatary3) > currentInput)
        {
            //direction = 1;
            neutralizeDirection = false;
            direction -= Time.fixedDeltaTime * rotatationSmoothness;
        ...
        print(direction);
        //m_Rigidbody.transform.forward = originPoint.transform.forward;
        angularRot = new Vector3(0, rotationSpeed * direction * speedMultiplier, 0);
        Quaternion deltaRotation = Quaternion.Euler(angularRot * Time.fixedDeltaTime);
        m_Rigidbody.MoveRotation(m_Rigidbody.rotation * deltaRotation);

        //maxRotation = rotationSpeed * direction * speedMultiplier * Time.fixedDeltaTime;

        //maxRotation = Mathf.Clamp(maxRotation, -90, 90);
        //angularRot = new Vector3(0, maxRotation, 0);
        //print(angularRot);
        //Quaternion rot = Quaternion.Euler(angularRot);
